...
main(int argc, char **argv)
{
	...
	uint8_t *old_compat = NULL;
	size_t old_compat_len;
	uint8_t *new_compat = NULL;
	size_t new_compat_len;
	int compat_mask = CRYPTO_KEYMASK_HMAC_CHUNK | CRYPTO_KEYMASK_HMAC_NAME | CRYPTO_KEYMASK_HMAC_CPARAMS;
	...
	/* Check key compatibility before touching any network or cache state. */
	if (keyfile_read(nkeyfile, &nmachinenum, compat_mask, 0, PASSPHRASE_TTY_STDIN, NULL) ||
	    crypto_keys_missing(compat_mask) != NULL ||
	    crypto_keys_export(compat_mask, &new_compat, &new_compat_len)) {
		warn0("Cannot load compatibility keys from %s", nkeyfile);
		exit(1);
	}
	if (keyfile_read(okeyfile, &omachinenum, compat_mask, 0, PASSPHRASE_TTY_STDIN, NULL) ||
	    crypto_keys_missing(compat_mask) != NULL ||
	    crypto_keys_export(compat_mask, &old_compat, &old_compat_len)) {
		warn0("Cannot load compatibility keys from %s", okeyfile);
		exit(1);
	}
	if (new_compat_len != old_compat_len || memcmp(new_compat, old_compat, new_compat_len) != 0) {
		warn0("new-key-file is not compatible with old-key-file\n"
		    "    (it was not generated by tarsnap-keyregen from it); refusing to\n"
		    "    proceed, since continuing would delete data which the new key set\n"
		    "    cannot address.");
		exit(1);
	}
	free(new_compat);
