/*
 * Regression for storage_delete_file() pending-count ownership.
 *
 * Include the real storage_delete.c and provide bounded local netpacket/event
 * fixtures.  A rejected operation must not remain pending and its cookie must
 * be released; otherwise storage_delete_flush() waits for a completion which
 * can never exist.
 */
#include "platform.h"
#include <sys/types.h>
