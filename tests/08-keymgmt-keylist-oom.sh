...
PRELOAD_EOF
	...
	if [ "${preload_kind}" = "linux" ]; then
		"${CC:-cc}" -shared -fPIC -o "${preload_library}" "${preload_source}" 2> "${compile_stderr}"
	else
		"${CC:-cc}" -dynamiclib -o "${preload_library}" "${preload_source}" 2> "${compile_stderr}"
	fi
	compile_status=$?
	...
	if [ "${preload_kind}" = "linux" ]; then
		LD_PRELOAD="${preload_library}" "${bindir}/tarsnap-keymgmt" --keylist=31,30 2> "${oom_stderr}"
	else
		DYLD_INSERT_LIBRARIES="${preload_library}" DYLD_FORCE_FLAT_NAMESPACE=1 "${bindir}/tarsnap-keymgmt" --keylist=31,30 2> "${oom_stderr}"
	fi
	keymgmt_status=$?
