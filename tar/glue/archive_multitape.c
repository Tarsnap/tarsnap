 ...
 *     argv, printstats, dryrun, creationtime, csv_filename, siginfo_cookie,
 *     siginfo_newbytes):
 ...
 * If ${csv_filename} is given, write statistics in CSV format.  If
 * ${siginfo_newbytes} is not NULL, call it with ${siginfo_cookie} and the
 * number of new bytes.  Return a cookie which can be passed to the multitape
 * layer.
 */
void *
archive_write_open_multitape(struct archive * a, uint64_t machinenum,
    ...
    const char * csv_filename, void ** siginfo_cookie,
    void siginfo_newbytes(void **, uint64_t))
{
	...
	if ((d = writetape_open(machinenum, cachedir, tapename,
	    ...
	    csv_filename, siginfo_cookie, siginfo_newbytes)) == NULL) {
